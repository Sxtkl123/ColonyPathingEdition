...
public interface AbstractEntityAIBasicAccessor<B extends AbstractBuilding> {
    ...
    @Accessor(value = "walkTo", remap = false)
    BlockPos getWalkTo();

    @Final
    @Invoker(value = "walkToBuilding", remap = false)
    boolean invokeWalkToBuilding();

    @Final
    @Invoker(value = "walkToSafePos", remap = false)
    boolean invokeWalkToSafePos(final BlockPos blockpos);

    @Final
    @Invoker(value = "setDelay", remap = false)
    void invokeSetDelay(final int timeout);

    @Invoker(value = "getInventory", remap = false)
    InventoryCitizen invokeGetInventory();

    @Invoker(value = "getPrimarySkillLevel", remap = false)
    int invokeGetPrimarySkillLevel();

    @Invoker(value = "getSecondarySkillLevel", remap = false)
    int invokeGetSecondarySkillLevel();

    @Invoker(value = "getModuleForJob", remap = false)
    WorkerBuildingModule invokeGetModuleForJob();
